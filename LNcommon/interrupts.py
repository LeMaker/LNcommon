...
GPIO_INTERRUPT_DEVICE = "/sys/devices/platform/gpio-sunxi/gpio/gpio%d" % GPIO_INTERRUPT_PIN
...
class PortEventListener(object):
    ...
    ...
    >>> port = LNcommon.mcp23s17.GPIOA
    >>> listener = LNcommon.interrupts.PortEventListener(port)
    >>> listener.register(0, LNcommon.interrupts.IODIR_ON, print_flag)
    >>> listener.activate()
    """
...
class GPIOInterruptDevice(object):
    ...
    def gpio_interrupts_enable(self):
        ...
        try:
            ...
            set_gpio_interrupt_edge('falling')
        except Timeout as e:
...
def watch_port_events(port, chip, pin_function_maps, event_queue,
    ...
    :type chip: :class:`LNcommon.mcp23s17.MCP23S17`
    ...
    while True:
        ...
        if port == LNcommon.mcp23s17.GPIOA:
...
#         (LNcommon.core.GPIOA, LNcommon.core.GPIOB)
#     :type port: int
...
def set_gpio_interrupt_edge(edge='falling'):
    ...
    GPIO.setup(GPIO_INTERRUPT_PIN,GPIO.IN,GPIO.PUD_DOWN)
    GPIO.remove_event_detect(GPIO_INTERRUPT_PIN)
    
    while time.time() < time_limit:
        try:
            if edge is 'rising':
                GPIO.add_event_detect(GPIO_INTERRUPT_PIN,GPIO.RISING)
            elif edge is 'falling':
                GPIO.add_event_detect(GPIO_INTERRUPT_PIN,GPIO.FALLING)
            else:
                PyErr_SetString(PyExc_RuntimeError, "GPIO edge should be rising, falling, none");
                pass
